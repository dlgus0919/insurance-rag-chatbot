# 보험 문서 RAG 챗봇 — 프로젝트 계획서 (Alpha)

> 작성: 기획자 / 일자: 2026-04-30 / 대상 산출물: 알파 버전

## 1. 배경 및 목표

### 1.1 문제
보험사 직원은 1,429페이지짜리 「건강보험 행위 급여·비급여 목록표 및 급여 상대가치점수」 고시 문서에서 코드·점수·산정지침을 수작업으로 검색해야 한다. 페이지 수와 계층 구조의 복잡성 때문에 단순 Ctrl+F 검색은 비효율적이며, 동일 코드가 여러 부칙·예외와 얽혀 있어 맥락 파악이 어렵다.
